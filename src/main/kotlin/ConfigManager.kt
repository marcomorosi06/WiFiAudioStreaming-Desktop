...
object ConfigManager {
    ...
    val fields: List<CfgField> = listOf(
        ...
        CfgField("app.language", CfgKind.STRING, emptyList(), "UI Language code (e.g. auto, en, vi, it, es, fr, de, etc.)",
            { it.app.language }, { s, v -> appCopy(s) { copy(language = coerceString("app.language", v)) } }),
        CfgField("app.autoReconnect", CfgKind.BOOL, emptyList(), "Auto reconnect client on connection drop",
            { it.app.autoReconnectEnabled }, { s, v -> appCopy(s) { copy(autoReconnectEnabled = coerceBool("app.autoReconnect", v)) } }),
        CfgField("app.savedServers", CfgKind.STRING_LIST, emptyList(), "Saved servers list",
            { it.app.savedServers.map { it.serialize() } }, { s, v -> appCopy(s) { copy(savedServers = coerceStringList("app.savedServers", v).mapNotNull { SavedServerItem.deserialize(it) }) } }),
        CfgField("app.recentServers", CfgKind.STRING_LIST, emptyList(), "Recent servers list",
            { it.app.recentServers }, { s, v -> appCopy(s) { copy(recentServers = coerceStringList("app.recentServers", v)) } }),
